...
> SDK leads with `agent-passport-system/core` (24 curated functions). MCP leads with `APS_PROFILE=essential` (142 tools). Full surface area — 123 modules, 142 MCP tools, TypeScript + Python SDKs — still available on the root import and `APS_PROFILE=full`.
...
| **Passport Deep-Dive** | [`passport.html`](https://aeoess.com/passport.html) | Full protocol surface with code examples, test counts, MCP tool listings, stats block. 123 modules documented in depth. | Engineers integrating the SDK |
...
|------|-----|------------------|
| `llms.txt` | [aeoess.com/llms.txt](https://aeoess.com/llms.txt) | Compact protocol summary — positioning, Core subpath, essential MCP profile, quick start. Full surface (123 modules, 142 MCP tools) listed as reference. ~5KB. Optimized for context-window efficiency. |
...
|------|-------------|------|---------|
| [**agent-passport-system**](https://github.com/aeoess/agent-passport-system) | [`agent-passport-system`](https://www.npmjs.com/package/agent-passport-system) v2.1.0 | SDK — 84 core + 39 v2 constitutional modules. Ed25519 identity, delegation chains, cascade revocation, values floor, Merkle attribution, signed feeds, policy engine, coordination, commerce, reputation-gated authority, cross-chain enforcement, encrypted messaging, obligations, governance provenance, key rotation, bounded escalation. 2366 tests, 609 suites. | Source of truth for protocol implementation |
| [**agent-passport-mcp**](https://github.com/aeoess/agent-passport-mcp) | [`agent-passport-system-mcp`](https://www.npmjs.com/package/agent-passport-system-mcp) v3.0.0 | MCP server — 20 essential tools by default (identity, delegation, enforcement, commerce, reputation). `APS_PROFILE=full` for the full 154-tool surface. Works with any MCP client: Claude Desktop, Cursor, Windsurf. | Source of truth for MCP tool surface |
