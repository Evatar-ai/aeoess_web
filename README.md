...
> SDK leads with `agent-passport-system/core` (24 curated functions). MCP leads with `APS_PROFILE=essential` (146 tools). Full surface area — 123 modules, 146 MCP tools, TypeScript + Python SDKs — still available on the root import and `APS_PROFILE=full`.
...
|------|-----|------------------|
| `llms.txt` | [aeoess.com/llms.txt](https://aeoess.com/llms.txt) | Compact protocol summary — positioning, Core subpath, essential MCP profile, quick start. Full surface (123 modules, 146 MCP tools) listed as reference. ~5KB. Optimized for context-window efficiency. |
